...
import {advanceTimeTo} from './utils';
...
describe("LiquidityLockerTest", function () {
  ...
  async function deployNewLiquidityLocker(secondsToRelease=1){
    ...
    // send eth
    const ethExpected = await liquidityLocker.totalEthExpected();
    ...
    const tokenExpected = await liquidityLocker.totalTokensExpected();      
  ...
  it("Can Deploy LiquidityLocker", async function () {
    ...
    try {
      const [owner, secondAcct, thirdAcct] = await ethers.getSigners();
      now = new Date();
      OneSecondsTime = now.setSeconds(now.getSeconds() + 1);

      // address dexRouterAddress, address token, address owner, uint256 price,uint256 releaseTime, uint liquidityPercentOfRaisedFunds,uint maxRaisedFunds
  ...
  it("Releases LP tokens after Release time", async () => {
    ...
    try {
      ...
      console.log('bal1: ', ( await lpToken.balanceOf(lockOwner)).toString() );
      // eslint-disable-next-line prettier/prettier
      console.log('bal: ', ethers.utils.parseEther( (await lpToken.balanceOf(lockOwner)).toString() ) );
      ...
      console.log(lockOwnerBalance,', ', lockOwnerPrevBalance);
  ...
  //     const expectedTokens = await locker.totalTokensExpected();
